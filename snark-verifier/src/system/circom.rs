...
/// Convert a [`VerifyingKey`] of [`circom`] into [`PlonkProtocol`].
pub fn compile<M: MultiMillerLoop>(vk: &VerifyingKey<M>) -> PlonkProtocol<M::G1Affine> {
    ...
    let quotient = {
        ...
        let numerator: Expression<M::Scalar> = powers_non_field(alpha.clone())
            .zip(constraints.iter())
            .map(|(power_of_alpha, constraint)| power_of_alpha * constraint)
            .sum();
        QuotientPolynomial {
            num_chunk: numerator.degree() - 1,
            numerator,
            chunk_degree: 1,
        }
    };
